...
export default function App() {
  ...
  const toggleCat = (id) => setVisibleIds(prev => prev.includes(id) ? (prev.length > 1 ? prev.filter(x => x !== id) : prev) : [...prev, id])
      ...
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: '14px 20px', position: 'sticky', top: 0, zIndex: 200 }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-0.03em', background: `linear-gradient(120deg,${C.lavender},${C.mint},${C.peach})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>🌌 CURIO BRIEF</div>
              <div style={{ fontSize: 9, color: C.muted, letterSpacing: '0.14em', textTransform: 'uppercase', fontFamily: 'monospace', marginTop: 2 }}>{reportName}</div>
            </div>
            <XPBar xp={xp} />
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ position: 'relative' }}>
              <button onClick={() => { setShowDates(s => !s); setShowFilter(false) }} style={{ padding: '7px 14px', background: C.card, border: `1px solid ${C.mint}44`, borderRadius: 8, color: C.mint, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                📅 {fmtDate(activeDate)} ▾
              </button>
              {showDates && (
                <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12, zIndex: 300, minWidth: 200, display: 'flex', flexDirection: 'column', gap: 4, boxShadow: '0 8px 32px #00000066' }}>
                  ...
                  {AVAILABLE_DATES.slice().reverse().map(d => (
                    <button key={d} onClick={() => { setActiveDate(d); setShowDates(false); setActiveId('news') }} style={{ padding: '8px 12px', borderRadius: 8, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', background: activeDate === d ? `${C.mint}22` : 'transparent', border: `1px solid ${activeDate === d ? C.mint : 'transparent'}`, color: activeDate === d ? C.mint : C.text, fontWeight: activeDate === d ? 700 : 400 }}>
                      {fmtDate(d)}
                    </button>
                  ))}
                  <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 6, paddingTop: 6, fontSize: 11, color: C.dim, lineHeight: 1.5 }}>More dates added daily!<br/>Ask Claude to generate.</div>
                </div>
              )}
            </div>
            <div style={{ position: 'relative' }}>
              <button onClick={() => { setShowFilter(s => !s); setShowDates(false) }} style={{ padding: '7px 14px', background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, color: C.muted, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
              ...
              {showFilter && (
                <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12, zIndex: 300, width: 280, display: 'flex', flexWrap: 'wrap', gap: 6, boxShadow: '0 8px 32px #00000066' }}>
                  {DOMAINS.map(d => (
                    <button key={d.id} onClick={() => toggleCat(d.id)} style={{ padding: '5px 10px', borderRadius: 8, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', background: visibleIds.includes(d.id) ? `${d.color}22` : C.surface, border: `1px solid ${visibleIds.includes(d.id) ? d.color : C.border}`, color: visibleIds.includes(d.id) ? d.color : C.muted }}>
                      {d.emoji} {d.label}
                    </button>
                  ))}
                </div>
            ...
            <div style={{ marginLeft: 'auto', fontSize: 11, color: C.muted }}>{AVAILABLE_DATES.length} brief{AVAILABLE_DATES.length !== 1 ? 's' : ''} available</div>
          </div>
          <div style={{ display: 'flex', gap: 2, marginTop: 12, overflowX: 'auto', paddingBottom: 1 }}>
            {visibleDomains.map(d => (
              <button key={d.id} onClick={() => setActiveId(d.id)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', borderRadius: '8px 8px 0 0', border: `1px solid ${activeId === d.id ? d.color + '66' : C.border}`, borderBottom: activeId === d.id ? `2px solid ${d.color}` : `1px solid ${C.border}`, background: activeId === d.id ? `${d.color}12` : 'transparent', cursor: 'pointer', fontSize: 11, fontWeight: activeId === d.id ? 800 : 500, color: activeId === d.id ? d.color : C.muted, whiteSpace: 'nowrap', fontFamily: 'inherit', transition: 'all 0.15s', flexShrink: 0 }}>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 16px 80px' }}>
        {active && brief && (
          <div style={{ marginBottom: 20, padding: '14px 18px', background: `${active.color}0d`, border: `1px solid ${active.color}33`, borderRadius: 14 }}>
            ...
            <div style={{ fontSize: 12, color: C.muted }}>{active.desc} · {fmtDate(activeDate)}</div>
          </div>
        )}
        {!brief && (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: C.card, borderRadius: 16, border: `1px solid ${C.border}` }}>
            ...
            <div style={{ fontSize: 14, color: C.muted, maxWidth: 360, margin: '0 auto', lineHeight: 1.8 }}>Ask Claude: <strong style={{ color: C.lavender }}>"generate brief for [date]"</strong></div>
            <div style={{ marginTop: 16, fontSize: 12, color: C.dim }}>Available: {AVAILABLE_DATES.map(d => fmtDate(d)).join(', ')}</div>
          </div>
        )}
        {brief && Renderer && data && <Renderer data={data} color={active.color} onXP={(earned) => setXp(x => x + earned)} />}
        {brief && (!Renderer || !data) && <div style={{ textAlign: 'center', padding: '40px', color: C.muted, fontSize: 14 }}>Content not available for this section.</div>}
      </div>
      {(showFilter || showDates) && <div onClick={() => { setShowFilter(false); setShowDates(false) }} style={{ position: 'fixed', inset: 0, zIndex: 100 }} />}
      <style>{`* { box-sizing: border-box; } ::-webkit-scrollbar { width: 4px; height: 4px; } ::-webkit-scrollbar-track { background: ${C.bg}; } ::-webkit-scrollbar-thumb { background: ${C.dim}; border-radius: 2px; } button { transition: opacity 0.15s; } button:hover:not(:disabled) { opacity: 0.8; }`}</style>
    </div>
  )
}
